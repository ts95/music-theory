import type { Etude } from '../contracts'

/** The selectable study exercises, in display order. */
export const ETUDES: Etude[] = [
  {
    id: 'relative-minors',
    section: 'Keys & Scales',
    number: 1,
    title: 'Relative Minors',
    subtitle: 'Major keys and their relative minors, on the circle of fifths',
  },
  {
    id: 'scales',
    section: 'Keys & Scales',
    number: 2,
    title: 'Scales',
    subtitle: 'Spell major & minor scales — plus the modes at Expert',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 2,
  },
  {
    id: 'scale-play',
    section: 'Keys & Scales',
    number: 3,
    title: 'Play the Scale',
    subtitle: 'Play the scale ascending — by keyboard or MIDI',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 2,
  },
  {
    id: 'key-signatures',
    section: 'Keys & Scales',
    number: 4,
    title: 'Key Signatures',
    subtitle: 'Which notes are sharp or flat in each key',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 1,
  },
  {
    id: 'chords',
    section: 'Chords & Harmony',
    number: 5,
    title: 'Chords by Degree',
    subtitle: 'Diatonic triads (and V7) in every major & minor key',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 2,
  },
  {
    id: 'chord-recognition',
    section: 'Chords & Harmony',
    number: 6,
    title: 'Chord Recognition',
    subtitle: 'Read a chord on the staff and name it — against the clock',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 2,
  },
  {
    id: 'chord-spelling',
    section: 'Chords & Harmony',
    number: 7,
    title: 'Spell the Chord',
    subtitle: 'Read a chord symbol and spell its notes',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 2,
  },
  {
    id: 'progressions',
    section: 'Chords & Harmony',
    number: 8,
    title: 'Progressions',
    subtitle: 'Roman-numeral progressions → concrete chords',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 2,
  },
  {
    id: 'intervals-ear',
    section: 'Ear Training',
    number: 9,
    title: 'Intervals by Ear',
    subtitle: 'Hear an interval in key and name it',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 3,
  },
  {
    id: 'progressions-ear',
    section: 'Ear Training',
    number: 10,
    title: 'Progressions by Ear',
    subtitle: 'Hear a progression over its tonic and name it',
    version: 2,
  },
  {
    id: 'melodic-dictation',
    section: 'Ear Training',
    number: 11,
    title: 'Melodic Dictation',
    subtitle: 'Hear a motif over its tonic and name it in solfège',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 2,
  },
  {
    id: 'rhythm-dictation',
    section: 'Ear Training',
    number: 12,
    title: 'Rhythm Dictation',
    subtitle: 'Hear a one-bar rhythm and pick the matching notation',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 3,
  },
  {
    id: 'rhythm-tap',
    section: 'Ear Training',
    number: 13,
    title: 'Tap the Rhythm',
    subtitle: 'Read a one-bar rhythm and tap it in time',
    levels: ['Easy', 'Medium', 'Hard', 'Expert'],
    version: 5,
  },
]
